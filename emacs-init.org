...
Emacs has 2 bindings with slightly different meaning that duplicate =Enter=:
=C-j= and =C-m=. That's a good thing, because =Enter= is a frequently used, but
a pretty distant key.
...
everything was finished for them. And they came up with their evil plan.
...
Stallman is so opposed to them. Besides, what a disgusting, smug name for a
mode - VIP.
...
need all 2/3/4, which are about the same? Especially, given those bindings are
quite attractive - brief and convenient - something you have a shortage of in
Emacs.
...
2. Swap buffers in windows
   ...
   of them. Similar to how you can use consecutive invocations of
...
it's tedious to pop those by one. So, I have a simple wrapper on =C-M-\= - it
works as =C-u C-SPC=, but ignores duplicate marks. If you provide it a universal
argument, it works as =C-x C-SPC= in that case.
...
fundamental to the design of this feature. In short, what does it mean to
"expand the region"? It means that we try to mark some bigger region around the
currently selected one (or around the point if no region is selected yet). For
this we use different rules, like "mark everything inside quotes", "mark
everything inside quotes, including the quotes", "mark everything inside
parenthesis", "mark a statement" and etc. We choose the "best" rule for the
currently marked region and use it as a next expansion. The "best" rule must
contain the original region, and expand it "as little as possible".
