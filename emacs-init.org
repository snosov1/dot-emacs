...
If you have a selected region and perform a delete, yank or insert operation,
the selected region should be deleted. It's not a default Emacs behavior, and if
you think about it, the default behavior seems to make more sense. But,
practically, everybody else has it by default, and the whole thing is not that
big of a deal, so what the hell.
...
Then, if you select a region that lies on a single line entirely, then
incremental searching (=C-s= and =C-r=) will use it as an initial value and make
a first jump. The common way I use it is:
...
will displayed in the echo-area after some time. But I don't see any reason why
...
file or =C-x f=. Also, sometimes I simply made a typo. Making both bindings to
do the same turned out to be a reasonable solution, which I applied in other
cases as well.
...
While we're at it, let's make =tar-mode= more confident in reverting buffers.
...
all logs have in common is that they are most often cumulative. So, you probably
...
Nothing special, really. Here, I mostly disable the mode-local bindings, so
global bindings are used instead.
...
- There can exist only one compile buffer at a given moment. So, if a
  compilation is in progress, another execution will ask you if you want to
  terminate it. But you can have arbitrarily many asynchronous shell commands at
  any time.
- Compile buffer is read-only and async-shell buffers are editable.
- Compile buffer colors the output and parses the output to be able to jump to
  source code. Async-shell buffer doesn't do anything fancy.
...
successive pressing expands to the contents of the quotes =abc def=, then it
marks the quotes, and so on:
...
think the author was carried away a little bit.
...
awkward expansions sometimes. Not always, but often enough to feel
uncomfortable, because I can't predict its result.
...
use it as a next expansion. "Best" rule must contain the original region, and
expand it as little as possible.
...
Currently, it will always chooses the region, that expands to the left as little
as possible. I tried different heuristics, but neither was good enough -
sometimes I wanted a "left" expansion, and sometimes I wanted a "right" one.

So, after using =expand-region= for a while, I think of it only in "lisp"-alike
cases - marking words, strings (quotes) and parenthesis.
...
I think I added the following function even before =expand-region= was
implemented. It puts quotes around the word at point. I thought =expand-region=
will make it obsolete and I will stop using it eventually. But it didn't happen.
I got used to it so much, that my fingers simply refuse to accept the 2
characters =expand-region= alternative.
...
The problem we're going to talk about today is "jump to definition". Unlike,
...
back. If you jumped not to the definition you wanted, you can provide a
universal argument, like, =C-u M-.=, to jump to the next candidate. When I can't
get to the desired definition after several jumps, I resort to =M-x
...
But I found that lack of thereof spoils first impression for people, that try it
out. And I want this configuration to provide more or less intuitive behavior
out-of-the-box to not scare those people. I don't go as far as enabling CUA
mode, because it would hurt the overall productivity. But I try to gather
...
Personally, I use =C-/= for undo and over time I found, that I don't really use
the default =C-z= binding. So I figured, why not to make it undo as well.
...
near the border. Or you can move the cursor using =C-v= and =M-v=. Moving the
cursor by paragraph is something in between those 2 options.
...
- Backup files (end with =~=) - second-to-last saved version. It's possible to
  save those in a separate directory, so they don't create clutter.
