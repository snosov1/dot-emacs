...
But there's even more. There are 2 functions =remote-authorize= and
=remote-enable-dired=.

If you execute those for some remote host, then:
...
- =M-x remote-authorize= will add your public key to the authorized keys list on
  ...
  #+END_EXAMPLE
- =remote-enable-dired= will modify the ".profile" on the remote, so that when
  you'll press =C-x C-j= for a =dired-jump= in the remote terminal (opened with
  =M-x remote-term=), it will open dired for the remote directory.
...
Emacs and your configuration on the remote. You will simply your local Emacs
instance.

Needless to say, you can do stuff, like, copying files from remote dired buffer
to local dired buffer will work transparently - no need for =scp=.

And all of this is enabled with just 3 simple steps:

- add a record to =~/.ssh/config= and re-open Emacs or call
  =M-x term-parse-ssh-config=
