...
(AST) directly. But I don't write a lot of Lisp. I even don't write a lot of
...
So, Emacs windows and buffers system looked like an ancient design to me,
serving the same goals as the "tabs" design. It is always interesting to see
those designs, and The more I used it, the more I realized, that it was a way
better design for what it does.
...
are trying to solve. Also, I like the vanilla "horizontal" ido more, not the
"vertical" modification.

So, the only interesting thing I can tell about my =ido= setup is that buffer
switching is bound to =C-TAB=. The idea came from desktop browsers and one
thing - it's a shorter and easier alternative to =C-x b=. And another thing,
which was a nice surprise to me, but may be a controversy to others - it is not
representable by an ASCII sequence, so it won't work in a terminal.
...
It works for =C-x b= (which is an ASCII sequence), because =C-x= is handled
specially in =term= mode. So, any sequence starting with =C-x= would also work,
but those are not as convenient as =C-TAB=.
...
Not many people know, but the trend to add "i"s to words to make them look iCool
was popular in Emacs long before Apple has come about. Behold another precedent:
Ibuffer. Honestly, I don't use it much, but it's really nice to have when you
need it.
