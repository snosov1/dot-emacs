...
* Contents                                                   :noexport:TOC_1:
 ...
 - [[#smarter-alternatives-to-built-in-functionality-0]["Smarter" alternatives to built-in functionality]]
...
* "Smarter" alternatives to built-in functionality [0%]
...
4. Dedicate current window

   Also, there are cases, when you want

   #+BEGIN_SRC emacs-lisp
     (define-key global-map (kbd "\C-x \C-d")
       (defun toggle-current-window-dedication ()
         "Toggle the dedication of the current window to its current
     buffer. If the dedication is on, the buffer displayed in the
     window can't be changed."
         (interactive)
         (message "Dedication is %s"
...
It's also convenient to fold the tags in HTML-like languages. In such cases, I
use toggling of individual blocks with the same binding.
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key nxml-mode-map (kbd "\C-ch") 'hs-toggle-hiding)
#+END_SRC
...
This section is one of the most important in the whole configuration, but I will
put it in this modest place where nobody can find it. Here, I will tell you the
secret of all secrets and answer the question of all questions.

Today there's a lot of words is being said about motivation and
productivity. The demand for those words is pretty high. More and more people
nowadays notice that for some reason they don't do the things they consciously
know they should be doing. They postpone the important things and do whatever
stupid shit, like, reading "news" or watching funny images on the
Internet.
...
But what do you think they do "about" it? Do they stop doing stupid shit and
start doing what they're ought to? Nope, when they feel like they have to change
something about there lives, they start looking for "solutions" to their
"problem".

So, as you can guess, a lot of people propose their solutions. They invent
"techniques" and "methods" for time management and self control. They write and
sell books about it and give expensive talks, workshops and whatnot.

And I've read and tried a good share of them and I can safely say: "None of the
time management techniques worth a broken penny if you're not willing to do the
things on the list." The hardest part of every method is to actually (surprise!)
do the things you have to do, not to organize them in a fancy fashion. Most of
the time, you feel inspired only until you get to know the method. After that -
you're back to doing stupid shit.

Of course, planning and tracking progress is absolutely essential to get things
done and grow. So, I'm not saying that time management is useless or bad, quite
the contrary. But you should know full surely - 99.9% of the success will depend
on whether you will actually do the things you need to do. The impact of the
particular time management method on the result is diminishingly small.
...
5 minutes (using the same timer again). After that, he set the timer to 25
minutes again and worked on the second item following with a 5 minutes
break. After 4 pomodoros (2 hours) he took a longer (20 minutes) break.

Don't know how many pomodoros later, he became rich by selling funny-looking
kitchen timers with funny names and describing this procedure to other
people. Some well managed time, eh?

Luckily, you don't need any expensive equipment for this method if you're using
Emacs. In the beginning of the day you just create a heading in a dedicated org
file, like:
...
And hit =<f12>= when the cursor is on the item you plan to do now. It will start
the timer and notify you when the 25 minutes end. You're supposed to stay
focused on the task for the whole pomodoro and simply put new things on the list
if they arise instead of switching to them. After the pomodoro ends, a
...
- the tasks break-down and 25 minutes size of pomodoros save you from being
  overwhelmed
...
main beauty. Because of its simplicity, pomodoro is absolutely honest to you.

If it doesn't help your procrastination - it cannot be because of the
technique. It's just too dead simple to be capable of messing things up. If you
can't make it work, it means, that you're weak, you're miserable and you deserve
to fail. Now you know it and you have no excuse.
...
- Backup files (end with =~=) - second-to-last saved version.

  To create less litter, I prefer to save the backup files in a separate
  directory.
...
- Make gdb window dedicated (=(set-window-dedicated-p (get-buffer-window) t)=)

  #+BEGIN_SRC emacs-lisp
    (defadvice gdb-setup-windows (after gdb--make-dedicated activate)
      (set-window-dedicated-p (get-buffer-window gud-comint-buffer) t))
  #+END_SRC

- Use shorter bindings for flow control
  # look for gud--define-bindings
  - enter/exit
  - stepping
  ...
  - f9
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (global-set-key (kbd "<f8>")
                      (defun gud-break-set-remove (arg)
                        (interactive "P")
                        (if arg
      ...
      ;;           (defun gud--define-bindings ()
      ;;              )
      ;;           )
    #+END_SRC


- Make a nice interface for watching (print/display) expressions
- Figure out how to 'attach' to a process smoothly

MISC:
#+BEGIN_EXAMPLE
  If you ever change the window layout, you can restore the "many windows" layout
  by typing M-x gdb-restore-windows. To toggle between the many windows layout and
  a simple layout with just the GUD interaction buffer and a source file, type M-x
  gdb-many-windows.

  You may also specify additional GDB-related buffers to display, either in the
  same frame or a different one. Select the buffers you want by typing M-x
  ...
  with the menu bar, with the GDB-Windows and GDB-Frames sub-menus of the GUD
  menu.

  When you finish debugging, kill the GUD interaction buffer with C-x k, which
  will also kill all the buffers associated with the session. However you need not
  do this if, after editing and re-compiling your source code within Emacs, you
  wish to continue debugging. When you restart execution, GDB automatically finds
  the new executable. Keeping the GUD interaction buffer has the advantage of
  keeping the shell history as well as GDB's breakpoints. You do need to check
  that the breakpoints in recently edited source files are still in the right
  places.
#+END_EXAMPLE
