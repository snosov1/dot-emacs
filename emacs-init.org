...
information about all function and type definitions in the sources.

TODO:
- JUMP TO NEXT OCCURRENCE
- MORE ABOUT UPDATE FUNCTION
- CHECK CASE SENSITIVITY

To generate the index you can use =M-x update-tags-file= (=C-x C-u=). Note, that
the suggested command uses =-e= flag, which is not available in =ctags=,
packaged with Emacs - you need to use Exuberant Ctags. Most likely, it can be
found in your OS repositories (on Ubuntu, try =sudo apt-get install ctags=).

If the index was already generated, you can use =M-x visit-tags-table= (=C-x
C-v=) to simply load it. After the tags table is loaded you can:

- call =M-x find-tag= (=M-.=) to jump to definition and =M-x pop-tag-mark=
  (=M-*=) to jump back
- call =M-x tags-apropos= (=C-x C-l=) to find all tags matching the regexp
- call =M-x tags-reset-tags-tables= (=C-x C-t=) to forget about all of the
  loaded tags tables.

As you can see, the hard problem of indexing is not solved inside Emacs, but is
delegated to specialized third-party software (=ctags= in that case). And the
indexing program works offline, i.e. the index generation step is explicit -
...
but, probably, you won't do this very often. The resulting tags file was about 1
Gb, but it took roughly 2 seconds to jump to a definition. So, it's absolutely
usable.

Also, I carried out this experiment for academic purposes only. In practice, you
won't need an index of the whole tree. Most likely, you will work on some
sub-project and you can index only the code you're working on.
