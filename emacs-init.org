...
Emacs has a strange convention for binding =C-x C-q= to toggle a read-only
...
functions, and also, you can use shorter bindings (without =C-= modifier) for
common operations.
...
viewing modes all the time? Wierd guys."
