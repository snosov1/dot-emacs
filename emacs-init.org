...
command found in my =.abbrev_defs= file and then I sent it to her.
...
like =(fov2focal (deg2rad 55))= and hit =C-x C-e=. At that day I became a tiny
...
#+BEGIN_SRC emacs-lisp
  (define-key global-map (kbd "C-M-\\")
    (defun pop-mark-jump (arg)
      ...
      (if arg
          (pop-global-mark)
        (setq mark-ring (remove-duplicates mark-ring :test 'equal))
        (set-mark-command '(4)))))
#+END_SRC
* Get full path
...
the first time you hear about it. I was also sceptic about it, like "What good
can you expect from the guys that invented minimap?".

But I saw a video by Magnar Sveen, where he showed-off his implementation of
multiple cursors in Emacs. I got the impression that he, himself, didn't really
know how to use them properly. But somehow it had a ring to him.
...
useful it was when I saw it, but I decided it give it a try.
...
I guess, I don't use a lot of the "stock" functionality and, probably, the way I
use multiple cursors is different from the way the author intended. But it
turned out to be so convenient, that now I can't imagine myself giving it up.

So, here's how I use multiple cursors.
...
The next important binding to add cursors is =M-@=:
...
- If the selected region is entirely on a single line, than it searches the
  ...
  of them.
- If the selected region spans multiple lines, then it adds a cursor on every
  line.

Now we're getting on speed. =M-#= adds successive numbers in the place of each
cursor. E.g. if you have 3 cursors, then pressing =M-#= will print 0 in the
position of the first cursor, 1 - in the position of the second one and 2 - in
the position of the third one. If you provide a prefix argument, say, =C-3 M-#=,
then the printed numbers will start with it - 3, 4, 5.
...
is bound to =eval-and-replace=) and get what we want.
...
"z" as the results. Only your imagination is the limit for =M-#= function.

BTW, I have this scary form =(char-to-string (+ ?x ))= in the abbrev table, so
all I have to do is type "char" and hit =M-/=.

Last, but not least - M-', which is my experimental function. It aligns all of
your cursors by adding the necessary number of spaces for each cursor.

If you feel overwhelmed by all the vast and tricky functionality this mode
provides - don't let it stop you from trying it out. Start with simple things,
like =C->= and =C-<=. Soon you will get pretty comfortable with it and then you
can start using other functions - little by little.
