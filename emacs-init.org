...
successive press expends to the contents of the quotes =abc def=, and then it
mark the quotes, and so on:
...
think that the author got a little bit carried away with it.

My guess is that it was started with editing of "lispy" code in mind (i.e. Lots
of Irritating Stupid Parenthesis). When you start expanding inside a deeply
nested form, it will expand to that form excluding parenthesis, then including
them, then it will expand to the containing form excluding parenthesis, then
including parenthesis and etc.

It works fine flawlessly in that use case, but then, apparently, the author
thought that applying it to a wider set of languages may be a good idea. Like,
if Emacs understands the syntax of the language - we can try expanding.

For example, if we have a statement, like =object.getHash();= and the cursor is
at the beginning of it, then we can extend something like =object=, =object.=,
=object.getHash=, =object.getHash()=, =object.getHash();=. It does sound kind of
neat, right? But in practice - it's not all roses. Not even close.

There are at least 2 reasons for that. The first one is that syntax of
LISP-languages is very simple. Basically, you're editing the Abstract Syntax
Tree (AST) directly, using parenthesis to denote node boundaries and nesting.
But the syntax of C-like languages (especially, C itself) is significantly more
complex. C grammar isn't even context-free! So, it makes writing precise
"expansion rules" unreasonable and which it leads to very awkward expansions. It
may not be a big deal for a lot of people, if it works accurately at least half
of the time. But I don't feel comfortable using a feature if I can't predict its
result.
...
little as possible, but it should still contain the original region.

But, for the =object.getHash();= example, try to define, which expansion is
better, if =getHash= is already selected - =object.getHash= or =getHash()=? If
you choose the first one, then you lose a chance to get the second expansion,
and if you choose the second one, you won't ever get the first one.
...
My experience showed that dots (sentences) and empty lines (paragraphs) were
causing most of the confusion. So, I dropped the rules, relying on them to make
=expand-region= dumber, but more predictable.

After using it for a while I think of =expand-region= when I need to mark words,
strings (quotes) and parenthesis. That's the most complex description I can keep
in my head.
